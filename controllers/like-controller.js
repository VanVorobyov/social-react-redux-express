...
const LikeController = {
    ...
    likePost: async (req, res) => {
        const {postId} = req.body; // Извлечение postId из тела запроса
        const userId = req.user.userId; // Извлечение userId из аутентифицированного пользователя

        // Проверка, указан ли postId
        if (!postId) {
            return res.status(400).json({error: 'Все поля обязательные для заполнения'}); // Возвращение ошибки, если postId отсутствует
        }
